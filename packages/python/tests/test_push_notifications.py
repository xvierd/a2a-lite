...
class TestLogPushNotifier:
    ...
    async def test_notify_custom_log_level(self, caplog):
        ...
        notifier = LogPushNotifier(level=logging.DEBUG)
...
def _verify_signature(body: bytes, secret: str, header_value: str) -> bool:
...
class TestWebhookPushNotifier:
    ...
    async def test_returns_successfully_on_2xx(self):
        ...
        for status in (200, 201, 204):
            ...
            notifier = WebhookPushNotifier(url="https://hooks.example.com/events", max_retries=0)

            with patch("httpx.AsyncClient", return_value=mock_client):
                await notifier.notify({"skill": "greet"})  # should not raise

    @pytest.mark.asyncio
    async def test_raises_on_4xx_after_retries(self):
        """notify() should raise after exhausting retries on 4xx responses."""
    ...
    @pytest.mark.asyncio
    async def test_raises_on_5xx_after_retries(self):
        """notify() should raise after exhausting retries on 5xx responses."""
    ...
    def test_secret_bytes_accepted(self):
        """WebhookPushNotifier should accept a bytes secret."""
        notifier = WebhookPushNotifier(
            url="https://hooks.example.com/events",
            secret=b"bytes-secret",
        )
        assert notifier._secret == b"bytes-secret"

    def test_secret_str_converted_to_bytes(self):
        """WebhookPushNotifier should convert a str secret to bytes."""
        notifier = WebhookPushNotifier(
            url="https://hooks.example.com/events",
            secret="str-secret",
        )
        assert notifier._secret == b"str-secret"
