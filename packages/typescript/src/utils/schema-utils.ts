...
export function isZodSchema(value: unknown): value is ZodTypeAny {
  ...
  if (obj._def && typeof obj._def === 'object' && obj._def !== null) {
    ...
    if (
      def.type &&
      ['string', 'number', 'boolean', 'object', 'array', 'enum', 'union', 'optional', 'nullable', 'default'].includes(
        def.type,
      )
    ) {
      return true;
...
export function zodToJsonSchema(schema: ZodTypeAny): Record<string, unknown> {
  ...
  switch (typeName) {
    ...
    case 'optional': {
      const inner =
        (schema as { _def?: { innerType?: ZodTypeAny } })._def?.innerType ||
    ...
    case 'ZodDefault':
    case 'default': {
      const defSchema = schema as {
        _def?: { innerType?: ZodTypeAny; defaultValue?: unknown | (() => unknown) };
        description?: string;
      };
      ...
      const defaultValue =
        typeof defaultValueRaw === 'function' ? (defaultValueRaw as () => unknown)() : defaultValueRaw;
      const result: Record<string, unknown> = {
        ...innerJson,
    ...
    case 'object': {
      const objSchema = schema as {
        ...
        _def?: { shape?: Record<string, ZodTypeAny> | (() => Record<string, ZodTypeAny>) };
      };
