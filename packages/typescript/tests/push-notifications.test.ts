...
describe('LogPushNotifier', () => {
  it('logs the event to console and does not throw', async () => {
    ...
    expect(parsed.skill).toBe('testSkill');
    expect(parsed.result).toEqual({ answer: 42 });

    consoleSpy.mockRestore();
  });

  it('logs the [PushNotifier] prefix', async () => {
    const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    const notifier = new LogPushNotifier();

    await notifier.notify(makeEvent());

    expect(consoleSpy.mock.calls[0][0]).toBe('[PushNotifier]');
    consoleSpy.mockRestore();
  });
});
...
describe('WebhookPushNotifier', () => {
  ...
  it('throws WebhookPushError immediately on 4xx without retrying', async () => {
    (fetch as ReturnType<typeof vi.fn>).mockResolvedValue(mockErrorResponse(400));

    const notifier = new WebhookPushNotifier({ url: WEBHOOK_URL, retryDelayMs: 0 });
    await expect(notifier.notify(makeEvent())).rejects.toThrow(WebhookPushError);

    // Should only have called fetch once (no retries for 4xx)
    expect(fetch).toHaveBeenCalledTimes(1);
  });

  it('throws WebhookPushError on 5xx after exhausting all retries', async () => {
  ...
  it('retries on network-level errors (fetch throws)', async () => {
  ...
  it('throws after all retries on persistent network errors', async () => {
    ...
    await expect(notifier.notify(makeEvent())).rejects.toThrow('timeout');
...
describe('Agent protocolTaskStore wiring', () => {
  ...
  it('builds the app using the provided protocolTaskStore', () => {
    // We verify the app builds without errors when a custom store is supplied.
    // Full integration would require a running server; this confirms wiring
    // doesn't throw at construction / buildApp time.
    const store = new InMemoryTaskStore() as unknown as import('@a2a-js/sdk/server').TaskStore;
...
describe('Agent pushNotifier auto-wiring', () => {
  ...
  it('does not register a completion hook when pushNotifier is absent', () => {
    ...
    // The agent card should report pushNotifications: false because no
    // completion hooks are registered.
    const card = agent.buildAgentCard();
    expect(card.capabilities?.pushNotifications).toBe(false);
  });

  it('notifier is called once per skill invocation', async () => {
    ...
    expect(fakeNotifier.notify).toHaveBeenCalledTimes(2);
  });
});
